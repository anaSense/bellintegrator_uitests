...
public class VacanciesTableComponent {
    ...
    private List<ElementsCollection> getVacanciesResultsFromAllPages() {
        ...
        assertThat(vacanciesTable.exists()).
                overridingErrorMessage(VacanciesConstants.EMPTY_VACANCIES_TABLE_ERROR).isTrue();
    ...
    public void checkSearchedVacanciesIsMatchToFilters(String specializationValue, String locationValue,
        ...
        assertThat(elementsCollections.isEmpty()).
                overridingErrorMessage(VacanciesConstants.EMPTY_VACANCIES_TABLE_ERROR).isFalse();
        for (int i = 0; i < elementsCollections.size(); i++) {
            ElementsCollection tmpCollection = elementsCollections.get(i);
            assertThat(tmpCollection.isEmpty()).
                    overridingErrorMessage(VacanciesConstants.EMPTY_VACANCIES_TABLE_ERROR).isFalse();
            for (int j = 0; j < tmpCollection.size(); j++) {
                if(specializationValue != null)
                    checkSpecialization(tmpCollection.get(j), specializationValue);
                if(locationValue != null)
                    checkLocation(tmpCollection.get(j), locationValue);
                if(isHot)
                    checkHot(tmpCollection.get(j));
                if(isRemote)
                    checkRemote(tmpCollection.get(j));
            }
        }
    }
}
